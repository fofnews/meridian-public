...
// Zoom-interpolated text-size for country-label: small enough at globe zoom (1)
// for Mapbox collision detection to spread labels, full editorial size at zoom 10.
const COUNTRY_LABEL_TEXT_SIZE = ['interpolate', ['linear'], ['zoom'], 1, 9, 4, 14, 7, 18, 10, 20];
...
function applyTypography(style) {
  ...
  // Single-font stacks only: Mapbox GL JS requests composite stacks as a single
  // URL path (e.g. "Playfair Display Bold,Noto Sans Regular/0-255.pbf") which
  // doesn't match the per-font directory layout generated by build-glyphs.js.
  setLayerLayout(style, 'country-label',   { 'text-font': ['Playfair Display Bold'] });
  setLayerLayout(style, 'continent-label', { 'text-font': ['Playfair Display Bold'] });
  // Cities — lighter serif
  setLayerLayout(style, 'settlement-major-label',       { 'text-font': ['Playfair Display Regular'] });
  ...
  setLayerLayout(style, 'state-label',                  { 'text-font': ['Noto Sans Bold'] });
